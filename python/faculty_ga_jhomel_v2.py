...
class FacultyLoadingGA:
                ...
                if (class_info['program_id'] == program_id and 
                    ...
                    if course['course_lecture'] > 0:
                        requirements.append({
                            ...
                            'duration': course['course_lecture']  # lecture hours
                        })
    ...
    def calculate_fitness(self, chromosome: Chromosome) -> float:
        ...
        for assignment in chromosome.assignments:
            ...
            faculty_load = self._calculate_faculty_load(assignment.faculty_id, chromosome.assignments)
            ...
            faculty_schedule[fid].append((assignment.time_slot, assignment.duration))
            ...
            room_schedule[rid].append((assignment.time_slot, assignment.duration))
            ...
            class_schedule[cid].append((assignment.time_slot, assignment.duration))
    ...
    def format_schedule(self, chromosome: Chromosome) -> str:
        ...
        for (class_id, course_id), assignments in sorted(schedule_by_class.items()):
            ...
            output.append(f"\n{'='*120}")
            output.append(f"Class: {class_info['set_name']} | Course: {course['course_code']}")
            output.append(f"Class Size: {class_info['class_size']} students")
            output.append(f"Course Units: Lecture={course['course_lecture']}, Laboratory={course['course_laboratory']}")
            output.append(f"{'-'*120}")
            ...
            for assignment in sorted(assignments, key=lambda x: x.time_slot):
                ...
                output.append(f"  Meeting Type: {assignment.meeting_type.upper()}")
                output.append(f"  Faculty: {faculty['name']} (Load contribution: {load_units:.2f} units)")
                output.append(f"  Room: {room['room_name']} (Capacity: {room['room_capacity']}, Type: {room['room_type']})")
                output.append(f"  Schedule: {days[day_idx]}, {start_hour:02d}:00 - {end_hour:02d}:00 ({assignment.duration} hours)")
                output.append(f"{'-'*120}")
        ...
        for fid, load in sorted(faculty_loads.items()):
            ...
            output.append(f"{faculty['name']}: {load:.2f} / {self.max_faculty_load} units {status}")
    ...
    def to_json(self, chromosome: Chromosome) -> dict:
        ...
        for set_name, schedule in by_set.items():
            ...
            schedule.sort(key=lambda x: (days.index(x['day']), x['start_hour']))
...
if __name__ == "__main__":
    ...
    # Run GA
    ga = FacultyLoadingGA(rooms, faculty, faculty_expertise, courses, classes, program_year_courses)
