    ...
    def create_individual(self) -> Chromosome:
        ...
        for req in self.teaching_requirements:
            ...
            if key in course_faculty_map:
                ...
                faculty_id = self._select_faculty_by_load(
                    qualified_faculty, assignments)
                # Remember for consistency
                course_faculty_map[key] = faculty_id

            # Select random suitable room
            suitable_rooms = self._get_suitable_rooms(meeting_type, class_size)
            room_id = random.choice(suitable_rooms) if suitable_rooms else random.choice(
            ...
            if random.random() < self.mutation_rate:
                ...
                if mutation_type == 0:  # Change faculty (must be qualified)
                    ...
                    suitable = self._get_suitable_rooms(
                        assignment.meeting_type, req['class_size'])
                    if suitable:
