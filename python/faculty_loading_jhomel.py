...
def find_suitable_room(rooms, course_type, institute_id, class_size, day, start_hour, 
    ...
    candidate_rooms = filter_rooms_by_type_and_institute(rooms, course_type, institute_id)
    
    # Sort by capacity (prefer rooms closer to class size)
    candidate_rooms.sort(key=lambda r: r.get("room_capacity", 0))  # Changed from "capacity"
...
def create_schedule(faculty_loads, rooms):
    ...
    for faculty_id, faculty_info in faculty_loads.items():
        ...
        for cls in faculty_info["assigned_classes"]:
            ...
            if lecture_hours > 0:
                print(f"  Scheduling LECTURE for {cls['course_code']} ({lecture_hours}h)...", end=" ")
            ...
            if lab_hours > 0:
                ...
                print(f"  Scheduling LABORATORY for {cls['course_code']} ({lab_hours} unit{'s' if lab_hours != 1 else ''} = {lab_contact_hours}h)...", end=" ")
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        for day in DAYS:
            ...
            for entry in day_schedule:
                ...
                f.write(f"Faculty     : {entry['faculty_name']} (ID: {entry['faculty_id']})\n")
                f.write(f"Room        : {entry['room_name']} (ID: {entry['room_id']})\n")
                f.write(f"Room Type   : {entry['room_type']}\n")
                f.write(f"Capacity    : {entry['room_capacity']} (Class Size: {entry['class_size']})\n")
...
def save_schedule_to_excel(schedule, unscheduled, faculty_loads, filename):
    ...
    sorted_schedule = sorted(schedule, key=lambda x: (DAYS.index(x["day"]), x["start_hour"]))
    ...
    sorted_by_faculty = sorted(schedule, key=lambda x: (x["faculty_name"], DAYS.index(x["day"]), x["start_hour"]))
    ...
    sorted_by_room = sorted(schedule, key=lambda x: (x["room_name"], DAYS.index(x["day"]), x["start_hour"]))
        ...
        for faculty in faculty_expertise_courses:
            ...
            if matches_expertise(course, faculty):
                ...
                if faculty_loads[faculty["faculty_id"]]["total_units"] + units <= MAX_LOAD:
                    ...
                    faculty_loads[faculty["faculty_id"]]["assigned_classes"].append(course)
                    faculty_loads[faculty["faculty_id"]]["total_units"] += units
                    faculty_loads[faculty["faculty_id"]]["total_lecture_hours"] += course["course_lec"]
                    faculty_loads[faculty["faculty_id"]]["total_lab_hours"] += course["course_lab"]
...
def save_faculty_load_to_text(faculty_loads: Dict, filename: str):
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        for fid, info in faculty_loads.items():
            ...
            f.write("-" * 80 + "\n")
            f.write(f"Total Units Load      : {info['total_units']:.2f} units\n")
            f.write(f"Total Lecture Hours   : {info['total_lecture_hours']} hours\n")
            f.write(f"Total Laboratory Hours: {info['total_lab_hours']} hours\n")
            f.write(f"Number of Classes     : {len(info['assigned_classes'])}\n\n")
        ...
        for fid, info in faculty_loads.items():
            ...
            if num_courses > 0:
                ...
                f.write(f"{info['faculty_name']:<40} | {num_courses:>2} courses | {total_units:>6.2f} units | {load_status}\n")
            else:
                total_faculty_without_loads += 1

        f.write("\n" + "-" * 80 + "\n")
        f.write(f"Total Faculty with Assignments   : {total_faculty_with_loads}\n")
        f.write(f"Total Faculty without Assignments: {total_faculty_without_loads}\n")
        f.write("=" * 80 + "\n")
...
if __name__ == "__main__":
    ...
    print(json.dumps(classes_course[:5], indent=2, default=str))  # preview only

    print("\nLoaded faculty_expertise:")
    print(json.dumps(faculty_expertise_courses[:5], indent=2, default=str))  # preview only

    # ------------------------------------------
    # APPLY FACULTY LOAD ASSIGNMENT
    # ------------------------------------------
    faculty_load_result = assign_faculty(classes_course, faculty_expertise_courses)
    print("\nLoaded faculty_expertise:")
    ...
    complete_schedule, unscheduled_meetings = create_schedule(faculty_load_result, rooms)
    ...
    schedule_excel_filename = os.path.join(output_dir, f"schedule_{timestamp}.xlsx")
    save_schedule_to_excel(complete_schedule, unscheduled_meetings, faculty_load_result, schedule_excel_filename)
